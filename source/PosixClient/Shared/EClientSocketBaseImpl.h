...
// event 10 = Added 'serverId' to the 'open order' & 'order status' events.
...
// in openOrder 17 = can receive liquidation field in exec reports, and
// notAutoAvailable field in mkt data 18 = can receive good till date field in
// open order messages, and send backfill requests 19 = can receive new extended
// order attributes in OPEN_ORDER 20 = expects TWS time string on connection
// after server version >= 20, and parentId in open order 21 = can receive bond
// contract details. 22 = can receive price magnifier in contract details 23 =
// support for scanner 24 = can receive volatility order parameters in open
// order messages 25 = can receive HMDS query start and end times 26 = can
// receive option vols in option market data messages 27 = can receive delta
// neutral order type and delta neutral aux price 28 = can receive option model
// computation ticks 29 = can receive trail stop limit price in open order and
// can place them: API 8.91 30 = can receive extended bond contract def, new
// ticks, and trade count in bars 31 = can receive EFP extensions to scanner and
// market data, and combo legs on open orders
//    ; can receive RT bars
// 32 = can receive TickType.LAST_TIMESTAMP
// 33 = can receive ScaleNumComponents and ScaleComponentSize is open order
// messages 34 = can receive whatIf orders / order state 35 = can receive contId
// field for Contract objects 36 = can receive outsideRth field for Order
// objects 37 = can receive clearingAccount and clearingIntent for Order objects
...
// TICK_OPTION_COMPUTATION 48 = can receive exemptCode in openOrder 49 = can
// receive hedgeType and hedgeParam in openOrder 50 = can receive
// optOutSmartRouting field in openOrder 51 = can receive
// smartComboRoutingParams in openOrder 52 = can receive deltaNeutralConId,
// deltaNeutralSettlingFirm, deltaNeutralClearingAccount and
// deltaNeutralClearingIntent in openOrder 53 = can receive orderRef in
// execution 54 = can receive scale order fields (PriceAdjustValue,
// PriceAdjustInterval, ProfitOffset, AutoReset,
//      InitPosition, InitFillQty and RandomPercent) in openOrder
// 55 = can receive orderComboLegs (price) in openOrder
// 56 = can receive trailingPercent in openOrder
// 57 = can receive commissionReport message
// 58 = can receive CUSIP/ISIN/etc. in
// contractDescription/bondContractDescription 59 = can receive evRule,
// evMultiplier in contractDescription/bondContractDescription/executionDetails
...
bool EClientSocketBase::DecodeField(bool& boolValue, const char*& ptr,
                                    const char* endPtr) {
  int intValue;
...
bool EClientSocketBase::DecodeField(int& intValue, const char*& ptr,
                                    const char* endPtr) {
...
bool EClientSocketBase::DecodeField(long& longValue, const char*& ptr,
                                    const char* endPtr) {
  int intValue;
  if (!DecodeField(intValue, ptr, endPtr)) return false;
  longValue = intValue;
...
bool EClientSocketBase::DecodeField(double& doubleValue, const char*& ptr,
                                    const char* endPtr) {
  if (!CheckOffset(ptr, endPtr)) return false;
  const char* fieldBeg = ptr;
  const char* fieldEnd = FindFieldEnd(fieldBeg, endPtr);
...
bool EClientSocketBase::DecodeField(IBString& stringValue, const char*& ptr,
                                    const char* endPtr) {
  if (!CheckOffset(ptr, endPtr)) return false;
  const char* fieldBeg = ptr;
...
bool EClientSocketBase::DecodeFieldMax(int& intValue, const char*& ptr,
                                       const char* endPtr) {
...
bool EClientSocketBase::DecodeFieldMax(long& longValue, const char*& ptr,
                                       const char* endPtr) {
  int intValue;
...
bool EClientSocketBase::DecodeFieldMax(double& doubleValue, const char*& ptr,
                                       const char* endPtr) {
  IBString stringValue;
  if (!DecodeField(stringValue, ptr, endPtr)) return false;
...
void EClientSocketBase::reqMktDepth(TickerId tickerId, const Contract& contract,
                                    int numRows,
...
void EClientSocketBase::reqHistoricalData(
    TickerId tickerId, const Contract& contract, const IBString& endDateTime,
...
void EClientSocketBase::reqRealTimeBars(
    TickerId tickerId, const Contract& contract, int barSize,
...
void EClientSocketBase::placeOrder(OrderId id, const Contract& contract,
                                   const Order& order) {
  // not connected?
  if (!m_connected) {
  ...
  //				m_pEWrapper->error( id, UPDATE_TWS.code(), UPDATE_TWS.msg()
  //+ 					"  It does not support SSHORT flag for combo legs."); 				return;
  //			}
  //		}
  //	}
  //}
  ...
  if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_CONID) {
    if (order.deltaNeutralConId > 0 ||
      ...
      m_pEWrapper->error(
          id, UPDATE_TWS.code(),
          UPDATE_TWS.msg() +
              "  It does not support deltaNeutral parameters: ConId, "
  ...
  if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_OPEN_CLOSE) {
    if (!IsEmpty(order.deltaNeutralOpenClose) || order.deltaNeutralShortSale ||
      ...
      m_pEWrapper->error(
          id, UPDATE_TWS.code(),
          UPDATE_TWS.msg() +
              "  It does not support deltaNeutral parameters: OpenClose, "
              "ShortSale, ShortSaleSlot, DesignatedLocation.");
      return;
    }
  }
  ...
  // order.stockRangeLower; double upper = (m_serverVersion == 26 && isVolOrder)
  // ? DBL_MAX : order.stockRangeUpper;
  ...
  //version 26 	double lower = (isVolOrder ? order.stockRangeLower : DBL_MAX);
...
void EClientSocketBase::reqAccountSummary(int reqId, const IBString& groupName,
                                          const IBString& tags) {
  // not connected?
  if (!m_connected) {
    m_pEWrapper->error(NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg());
