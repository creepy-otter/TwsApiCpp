...
class MyEWrapper : public EWrapperL0 {
 public:
  ...
  virtual void tickPrice(TickerId tickerId, TickType field, double price,
                         int canAutoExecute) {
    ///	int id = 10, y = 0; id = id/y; // Divide by zero to test
    ///EWrapperL0Protected 	int *id = 0; *id = 100;
    ...
    PrintProcessId,
        printf("TP: %4ld %02d:%02d:%02d %10s %5.3f\n", tickerId, _tm->tm_hour,
               _tm->tm_min, _tm->tm_sec, *(TickTypes::ENUMS)field, price);
  }
  ...
  virtual void tickSize(TickerId tickerId, TickType field, int size) {
    ...
    struct tm* _tm = localtime(&_t);
    PrintProcessId,
        printf("TS: %4ld %02d:%02d:%02d %10s %5d\n", tickerId, _tm->tm_hour,
               _tm->tm_min, _tm->tm_sec, *(TickTypes::ENUMS)field, size);
  }
  ...
  virtual void updateAccountValue(const IBString& key, const IBString& val,
    ...
    if (UAVK *= key)
      PrintProcessId,
          printf("AC: %s %5s   %-30s %s\n", (const char*)accountName,
                 (const char*)currency, *UAVK, (const char*)val);
    else
      PrintProcessId,
          printf("AC: %s %5s ? %-30s %s\n", (const char*)accountName,
                 (const char*)currency, (const char*)key, (const char*)val);
  }
  ...
  virtual void contractDetails(const ContractDetails& contractDetails) {
    ...
    PrintProcessId,
        printf("CD: %10s %5s %8s, %5.2f\n", (const char*)C.localSymbol,
               (const char*)C.secType, (const char*)C.expiry, C.strike);
  }
  ...
  virtual void historicalData(TickerId reqId, const IBString& date, double open,
    ...
    PrintProcessId,
        printf("HD: %4ld %10s %5.3f %5.3f %5.3f %5.3f %7d %7d\n", reqId,
  ...
  virtual void updateMktDepth(TickerId id, int position, int operation,
    ...
    PrintProcessId, printf("MD: %4ld %2d %10s %5s %7.2f %5d\n", id, position,
                           *(MktDepthOperation::ENUMS)operation,
                           *(MktDeptSide::ENUMS)side, price, size);
  }

  virtual void updateMktDepthL2(TickerId id, int position, IBString marketMaker,
...
void TestEnums(void) {
  ...
  for (UpdateAccountValueKey::iterator ac = UpdateAccountValueKey::begin();
       ac != UpdateAccountValueKey::end(); ++ac)
    printf("%4d %s\n", ac->first, ac->second);
}
...
int main(void) {
  ...
  */
  ...
  if (TEST(0, EC->eConnect("", 7496, 100))) {
    ...
    */
    EC->reqHistoricalData(20, C, EndDateTime(2014, 8, 4),
                          DurationStr(1, *DurationHorizon::Days),
                          *BarSizeSetting::_1_hour, *WhatToShow::TRADES, true,
                          FormatDate::AsDate);
    /*
    //		EC->reqMktData( 20, C, false );
                    ...
                    , C
                    , EndDateTime(2006,10,03), DurationStr(1, DH_Days),
    *BarSizeSetting::_1_secs , *WS_TRADES , true , FD_AsDate
                    );
    */
    {
      Contract C;
  ...
  while (id++ < 1000) {
    ...
    /* for 'stress' testing */
    if (0 == id % 50) TEST(id, EC->reqMktData(id, C, "", false));
    /**/
    if (30 == id) TEST(id, EC->eDisconnect());
    /**/
    if (40 == id) TEST(id, EC->eDisconnect());

    if (60 == id || 70 == id)
      TEST(id, EC->reqMktData(id, C, "", false));  // will fail of course

    if (130 == id) TEST(id, EC->eConnect("", 7496, 10));
    ...
    if (190 == id)
      TEST(id,
           EC->reqHistoricalData(id, C, EndDateTime(2013, 02, 20),
                                 DurationStr(1, *DurationHorizon::Days),
                                 *BarSizeSetting::_1_hour, *WhatToShow::TRADES,
                                 true, FormatDate::AsDate));

    if (200 == id) TEST(id, EC->reqMktData(id, C, "", false));

    /**/
    if (800 == id) TEST(id, EC->cancelMktData(200));
    /**/
  }
